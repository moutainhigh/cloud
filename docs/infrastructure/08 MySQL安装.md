...
##Docker 安装
```bash
    docker run --name mysql -p 13306:3306 -v /etc/localtime:/etc/localtime:ro -v /opt/docker/mysql/conf:/etc/mysql/conf.d -v /opt/docker/mysql/data:/var/lib/mysql -v /opt/docker/mysql/logs:/logs -e MYSQL_ROOT_PASSWORD=123456 -d mysql:5.7 --character-set-server=utf8mb4 --collation-server=utf8mb4_unicode_ci
```
