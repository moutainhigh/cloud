...
public class BaseDeveloperServiceImpl implements BaseDeveloperService {
    ...
    @Autowired
    public BaseDeveloperServiceImpl(AccountService accountService, BaseUserDomainService baseUserDomainService, BaseRoleDomainService baseRoleDomainService, BaseAuthorityDomainService baseAuthorityDomainService) {
    ...
    public UserAccount login(String account) {
        ...
        if (StringUtils.isNotBlank(loginType)) {
            baseAccount = accountService.getAccount(account, loginType, ACCOUNT_DOMAIN_ADMIN);
        } else {
            // 非第三方登录

            // 用户名登录
            baseAccount = accountService.getAccount(account, ACCOUNT_TYPE_USERNAME, ACCOUNT_DOMAIN_ADMIN);
            // 手机号登录
            if (com.smart4y.cloud.core.infrastructure.toolkit.StringUtils.matchMobile(account)) {
                baseAccount = accountService.getAccount(account, ACCOUNT_TYPE_MOBILE, ACCOUNT_DOMAIN_ADMIN);
            }
            // 邮箱登录
            if (com.smart4y.cloud.core.infrastructure.toolkit.StringUtils.matchEmail(account)) {
                baseAccount = accountService.getAccount(account, ACCOUNT_TYPE_EMAIL, ACCOUNT_DOMAIN_ADMIN);
            }
        }
        ...
        if (null != baseAccount) {
            ...
            // 添加登录日志
            accountService.loginLog(baseAccount);
        }
        return userAccount;
    }
