...
public class SaveOrUpdateProvider extends MapperTemplate {
    ...
    public String saveOrUpdate(MappedStatement ms) {
        ...
        // String fieldName = Kit.help().reflection().getFieldName(BaseEntity::getCreateDate);
        for (EntityColumn column : columnList) {
            if (!column.isInsertable() /*|| fieldName.equals(column.getEntityField().getName())*/) {
                // SaveOrUpdate 方法跳过 createDate 字段
                continue;
            }
            if (!column.isIdentity()) {
                sql.append(SqlHelper.getIfNotNull(null, column, column.getColumnEqualsHolder(null) + ",", true));
            }
        }
        sql.append("</trim>");
