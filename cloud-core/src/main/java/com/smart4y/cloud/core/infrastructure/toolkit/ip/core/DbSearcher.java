...
public class DbSearcher {
     ...
     * construct class
     */
    public DbSearcher(DbConfig dbConfig) {
        this.dbConfig = dbConfig;
        try {
    ...
    private String getDbFilePath() throws IOException {
        String resourcePath = "ip/ip2region.db";
        String dbPath = DbSearcher.class.getResource("/" + resourcePath).getPath();
        File file = new File(dbPath);
        if (!file.exists()) {
            InputStream asStream = DbSearcher.class.getClassLoader()
                    .getResourceAsStream(resourcePath);
            dbPath = System.getProperties().getProperty("java.io.tmpdir") + "ip.db";
